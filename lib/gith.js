...
var hookSecret=process.env.GITHUB_SECRET || 'thequickbrownfox';
var github_signature="";

var calculate_signature=function(payload_body,secret)  {
...
var Gith = function( eventaur, settings ) {
  ...
  eventaur.on( 'payload', function( originalPayload ) {
    ...
    //set new payload with signature information
    payload.github_signature=github_signature;
    payload.signatureCheck=calculate_signature(originalPayload,hookSecret);
    payload.signatureMatch= (payload.github_signature == payload.signatureCheck);
...
var listen = function( eventaur, port ) {
  ...
  this.server = http.createServer( function( req, res ) {
    ...
    if ( req.method === "POST" ) {
      ...
      github_signature = req.headers['x-hub-signature'];
    }
